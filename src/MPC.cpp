...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    // Any additions to the cost will be added to `fg[0]`.
    fg[0] = 0;

    // Reference State Cost
    // 1. The CTE and the direction error should be minimized
    for(int t = 0; t < N; t++)
    {
      fg[0] += Weight_cte*CppAD::pow(vars[cte_start + t],2);
      fg[0] += Weight_errorPsi*CppAD::pow(vars[epsi_start + t],2);
    ...
    for(int t = 0; t < N-2; t++)
    {
      fg[0] += Weight_lowAcutatorChanges * CppAD::pow((vars[delta_start + t + 1] - vars[delta_start + t]),2);
      fg[0] += Weight_lowAcutatorChanges * CppAD::pow((vars[a_start + t + 1] - vars[a_start + t]),2);
    }
    ...
    for (int t = 1; t < N; t++) {
      ...
      // f(x) is a polynom with dimenson of 3
      AD<double> f0 = coeffs[0] + coeffs[1] * x0 + coeffs[2] * x0 * x0 + coeffs[3] * x0 * x0 *x0;
      // desired psi is equal to arctan(f´(x0) )
      AD<double> psides0 = CppAD::atan( coeffs[1] + 2*coeffs[2] * x0 + 3*coeffs[3] * x0 * x0 );

      // set the equations for the model
